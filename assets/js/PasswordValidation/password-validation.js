	...
	$.fn.extend({
		passwordValidation: function(_options, _callback, _confirmcallback) {
			...
			function buildPasswordRegex() {
				...
				if(_options.noUpper) 	cases.push({"regex": "(?=" + CHARSETS.upperCaseSet + ")",  																				"message": "Password no puede contener mayúsculas"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.upperCaseSet + "][^" + CHARSETS.upperCaseSet + "]*").repeat(_options.minUpperCase) + ")", 	"message": "El password por lo menos debe contener " + _options.minUpperCase + " Letras mayúsculas."});
				if(_options.noLower) 	cases.push({"regex": "(?=" + CHARSETS.lowerCaseSet + ")",  																				"message": "El password no puede contener letras minusculas"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.lowerCaseSet + "][^" + CHARSETS.lowerCaseSet + "]*").repeat(_options.minLowerCase) + ")", 	"message": "El password por lo menos debe contener " + _options.minLowerCase + " Letras minusculas."});
				if(_options.noDigit) 	cases.push({"regex": "(?=" + CHARSETS.digitSet + ")", 																					"message": "El password no puede contener números"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.digitSet + "][^" + CHARSETS.digitSet + "]*").repeat(_options.minDigits) + ")", 			"message": "El password por lo menos debe contener " + _options.minDigits + " Digitos."});
				if(_options.noSpecial) 	cases.push({"regex": "(?=" + CHARSETS.specialSet + ")", 																				"message": "El password no puede contener caracteres especiales"});
				else 					cases.push({"regex": "(?=" + ("[" + CHARSETS.specialSet + "][^" + CHARSETS.specialSet + "]*").repeat(_options.minSpecial) + ")", 		"message": "El password debe contener al menos " + _options.minSpecial + " Caracteres especiales (! $ + - * , .)"});

				cases.push({"regex":"[" + charsetToString() + "]{" + _options.minLength + ",}", "message":"El password por lo menos debe contener " + _options.minLength + " caracteres."});
