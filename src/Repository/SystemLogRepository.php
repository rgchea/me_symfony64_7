 ...
 */
class SystemLogRepository extends \Doctrine\ORM\EntityRepository
{
    ...
    public function getRequiredDTData($start, $length, $orders, $search, $columns, $otherConditions)
    {
        ...
        // Create inner joins
        $query->join('sl.createdBy', 'u');
        $countQuery->join('sl.createdBy', 'u');

        $query->leftJoin('sl.guide', 'g');
        $countQuery->leftJoin('sl.guide', 'g');
        ...
        foreach ($columns as $key => $column)
        {
            if ($column['search']['value'] != '')
            {
                ...
                switch($column['name'])
                {
                    ...
                    case 'detail':
                        {
                            $searchQuery = 'sl.detail LIKE \'%'.$searchItem.'%\'';
                            break;
                        }

                    case 'guide':
                        {
                            $searchQuery = 'g.code LIKE \'%' . $searchItem . '%\'';
                        }
